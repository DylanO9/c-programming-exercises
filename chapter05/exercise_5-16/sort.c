...
{
    // Return 0 if s == t lexicographically
    // Return negative if s < t lexicographically
    // Return positive if s > t lexicographically
    for (; ; s++, t++) {
        if (directory_order) {
            while (*s && !isspace(*s) && !isalnum(*s)) {
                s++;
            }
            while (*t && !isspace(*t) && !isalnum(*t)) {
                t++;
            }
        }
        
        char cs = fold ? tolower(*s) : *s;
        char ct = fold ? tolower(*t) : *t;
