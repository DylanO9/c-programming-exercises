...
int main() {
    ...
    while ((type = getop(s)) != EOF) {
        ...
        case 'C':
            ...
            break; 
        case '=':
            op2 = pop(); // variable name, but we don't need
...
void ungets(char s[]) {
    ...
    // My approach: If we are just trying to push whatever is in s back into the buffer
    // Read s from back to front and store that in the buffer since getch looks at buf[--bufp]
    int len = strlen(s) - 1;
    for (int i = len; i >= 0; i--) {
        ungetch(s[i]);
    }
